...
static int wrn_open(struct net_device *dev)
{
	...
	printk(KERN_INFO "Ron - start wrn_open\n");
...
static int wrn_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	//	dev->trans_start = jiffies; /* N/A in new kernels */
...
{
	printk(KERN_INFO "Ron - start wrn_netops_init - setting netdev_ops\n");
...
static void __wrn_rx_descriptor(struct wrn_dev *wrn, int desc)
{
	...
	//	dev->last_rx = jiffies; /* N/A in new kernels */
	ep->stats.rx_packets++;
